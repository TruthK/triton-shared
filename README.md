...
chmod +x install_conda_dependencies.sh
./install_conda_dependencies.sh
...
mkdir build
cd build
cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_ASSERTIONS=ON ../llvm -DLLVM_ENABLE_PROJECTS="mlir;llvm" -DLLVM_USE_LINKER=lld -DLLVM_TARGETS_TO_BUILD="host;NVPTX;AMDGPU"
ninja
...
LLVM_INCLUDE_DIRS=$LLVM_BUILD_DIR/include   LLVM_LIBRARY_DIR=$LLVM_BUILD_DIR/lib   LLVM_SYSPATH=$LLVM_BUILD_DIR   TRITON_BUILD_WITH_CLANG_LLD=true python3 -m pip install --no-build-isolation -vvv '.[tests]'
```
