...
struct LoadToTransferReadConverter : public OpConversionPattern<tts::LoadOp> {
private:
  using OpConversionPattern<tts::LoadOp>::OpConversionPattern;

public:
  LoadToTransferReadConverter(const TypeConverter &typeConverter,
                              MLIRContext *context)
...
struct StoreToTransferWriteConverter
    : public OpConversionPattern<tts::StoreOp> {
private:
  using OpConversionPattern<tts::StoreOp>::OpConversionPattern;

public:
  StoreToTransferWriteConverter(const TypeConverter &typeConverter,
                                MLIRContext *context)
...
//   void createStackedCopies(Value block1, Value block2, Value dst, Location
//   loc,
//                            ConversionPatternRewriter &rewriter) const {
...
//   getSideBySideSubviews(ArrayRef<OpFoldResult> dims, Value block1, Value
//   block2,
//                         Location loc,
//                         ConversionPatternRewriter &rewriter) const {
...
//       rewriter.create<scf::IfOp>(loc, accBase, [&](OpBuilder &b, Location
//       loc) {
...
//     auto dstType = tensor::ExtractSliceOp::inferResultType(sourceType,
//     offsets,
//                                                            dims, strides);

//     return b.create<tensor::ExtractSliceOp>(loc, dstType, source, offsets,
//     dims,
//                                             strides);
//   }
...
void mlir::triton::populateStructuredToMemrefConversionPatterns(
  ...
  patterns.add<LoadToTransferReadConverter, StoreToTransferWriteConverter>(
      patterns.getContext());
