...
#include "triton-shared/Codegen/Dialect/Codegen/IR/IREECodegenOps.h"
  ...
  auto remapOpFoldResult = [&bvm](ArrayRef<OpFoldResult> ofrs) {
    ...
    for (auto ofr : ofrs) {
      if (isa<Attribute>(ofr)) {
        ...
        clonedOfrs.push_back(bvm.lookupOrDefault(ofr.dyn_cast<Value>()));
      }
    }
    return clonedOfrs;
  };
...
//   return cloneOffsetsSizesAndStridesImpl(
//       builder, loadOp, ValueRange{loadOp.getSource()},
//       loadOp.getMixedOffsets(), loadOp.getMixedSizes(),
//       loadOp.getMixedStrides(), loadOp.getSourceDims());
// }
...
//       mlir::getMixedValues(expandedStaticShape, expandedDynamicDims,
//       rewriter);
...
//     auto loadOp =
//     reshapeSrc.getDefiningOp<IREE::Flow::DispatchTensorLoadOp>(); if
//     (!loadOp)
//       return failure();
...
//     // In the corner case where the expand_shape is the source of a store,
//     dont
...
//         subspanOp.getLoc(), newSubspanType, subspanOp.getLayout(),
//         subspanOp.getBinding(), subspanOp.getByteOffset(),
//         expandedDynamicDims, subspanOp.getAlignmentAttr(),
//         subspanOp.getDescriptorFlagsAttr());
...
//   using
//   OpRewritePattern<IREE::Flow::DispatchTensorStoreOp>::OpRewritePattern;

//   LogicalResult matchAndRewrite(IREE::Flow::DispatchTensorStoreOp storeOp,
//                                 PatternRewriter &rewriter) const override {
//     // Make sure we are storing the full incoming subspan. Otherwise we
//     cannot
//     // simply adjust the subspan's resultant type later.
//     if (!isFullSlice(storeOp, storeOp.getTargetType(),
//                      storeOp.getTargetDims())) {
...
//     auto reshapeOp =
//     storeOp.getValue().getDefiningOp<tensor::ExpandShapeOp>(); if
//     (!reshapeOp) {
//       return failure();
//     }
...
//       if (isFullSlice(loadOp, loadOp.getSourceType(),
...
//             .getAccess();
//     auto newSubspanType =
//         IREE::Flow::DispatchTensorType::get(tensorAccess,
//         reshapeSrc.getType());
...
// /// Folds tensor.collapse_shape into the source
// hal.interface.binding.subspan.
// ///
// /// For example, this matches the following pattern:
// ///
...
// ///   %tensor = flow.dispatch.tensor.store %0, %subspan :
// ///       tensor<?xf32> ->
// !flow.dispatch.tensor<writeonly:tensor<?xf32>>{%dim}
// ///
// /// And turns it into:
// ///
// ///   %subspan = hal.interface.binding.subspan ... :
...
// /// (`FoldCollapseShapeIntoTensorInsertSlice`) handles cases where the slic
// is
// /// not a full slice, but requires the shapes to be static. This pattern
// handles
// /// dynamic shapes as well. Combine the two (if possible, it isnt clear that
// it
// /// is possible)
...
//         subspanOp.getLoc(), newSubspanType, subspanOp.getLayout(),
//         subspanOp.getBinding(), subspanOp.getByteOffset(),
//         expandedDynamicShape, subspanOp.getAlignmentAttr(),
//         subspanOp.getDescriptorFlagsAttr());
...
//     ArrayRef<int64_t> reshapeSrcShape =
//     collapseShape.getSrcType().getShape();

//     // Verify the subspan shape against the shape of the slice being
//     inserted. for (auto [size, group] : llvm::zip_equal(
//              subspanType.getShape(),
...
//                                               rewriter.getIndexAttr(1));
//     rewriter.replaceOpWithNewOp<IREE::Flow::DispatchTensorStoreOp>(
//         storeOp, collapseShape.getSrc(), newSubspanOp,
//         storeOp.getTargetDims(), expandedOffsets, expandedSizes,
//         expandedStrides);
//     return success();
//   }
// };
