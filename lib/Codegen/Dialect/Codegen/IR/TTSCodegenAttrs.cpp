...
#include "triton-shared/Codegen/Dialect/Codegen/IR/TTSCodegenAttrs.cpp.inc"
#include "triton-shared/Codegen/Dialect/Codegen/IR/LoweringConfigEnums.cpp.inc"
...
LogicalResult TranslationInfoAttr::verify(
    ...
    GPU::DispatchLoweringPassPipelineAttr passPipeline,
    SymbolRefAttr codegenSpec, ArrayRef<int64_t> workgroupSize,
  ...
  if (passPipelineValue > GPU::DispatchLoweringPassPipeline::None) {
    return emitError() << "invalid pass pipeline value : "
                       << stringifyEnum(passPipeline.getValue());
  }
  auto tdPassPipeline =
      GPU::DispatchLoweringPassPipeline::TransformDialectCodegen;
...
LogicalResult WorkgroupMappingAttr::verifyAttrList(MLIRContext *context,
  ...
  SmallVector<GPU::WorkgroupMappingAttr> mappingAttrs;
  llvm::SmallDenseSet<GPU::WorkgroupMappingAttr, 4> attrSet;
  auto emitError = mlir::detail::getDefaultDiagnosticEmitFn(loc);
  for (auto attr : attrs) {
    auto typedAttr =
        ::mlir::dyn_cast_or_null<GPU::WorkgroupMappingAttr>(attr);
    if (!attr) {
  ...
  if (mappingAttrs.front().getId() != GPU::WorkgroupId::IdX) {
  ...
  if (mappingAttrs[1].getId() != GPU::WorkgroupId::IdY) {
  ...
  auto mappingAttrsRef = ArrayRef<GPU::WorkgroupMappingAttr>(
...
//===---------------------------------------------------------------------===//
// tts_codegen.encoding_nop_layout
//===---------------------------------------------------------------------===//
...
void TTSCodegenDialect::initializeCodegenAttrs() {
  addAttributes<
#define GET_ATTRDEF_LIST
#include "triton-shared/Codegen/Dialect/Codegen/IR/TTSCodegenAttrs.cpp.inc" // IWYU pragma: keeep
      >();
}
...
//===----------------------------------------------------------------------===//
// Helpers for getting/setting tts_codegen.translation_info attribute on a
...
//===----------------------------------------------------------------------===//
// Helpers for getting/setting `tts_codegen.lowering_config` attribute on root
// operations.
// ===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Helpers for getting/setting `tts_codegen.compilation_info` attribute on root
