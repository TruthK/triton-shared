...
#include "triton-shared/Dialect/LinalgExt/Utils/Utils.h"
...
#define DEBUG_TYPE "tts-gpu-config-utils"
...
LogicalResult setDataTiledMultiMmaLoweringConfig(
  ...
  auto multiMmaOp = dyn_cast<TTS::GPU::MultiMmaOp>(op);
  if (!multiMmaOp) {
    return failure();
  }
  ...
  auto loweringConfig = TTS::GPU::LoweringConfigAttr::get(context, configDict);

  // Don't add any special padding or prefetching, since the data-tiled layout
  // is already what we want.
  SmallVector<NamedAttribute, 1> pipelineAttrs;
  auto pipelineOptions = TTS::GPU::GPUPipelineOptionsAttr::get(
      context, /*prefetchSharedMemory=*/false,
      /*no_reduce_shared_memory_bank_conflicts=*/true,
      /*use_igemm_convolution=*/false,
      /*reorder_workgroups_strategy=*/std::nullopt);
  pipelineAttrs.emplace_back(
      b.getStringAttr(TTS::GPU::GPUPipelineOptionsAttr::getDictKeyName()),
      pipelineOptions);
  ...
  return setOpConfigAndEntryPointFnTranslation(
      ...
      TTS::Codegen::DispatchLoweringPassPipeline::LLVMGPUTileAndFuse,
...
static std::optional<GPUMMASchedule> getMmaScheduleFromProblemAndTarget(
  ...
  // See https://github.com/TTS-org/TTS/issues/16341 for details.
...
setIGEMMConvolutionLoweringConfig(TTS::GPU::TargetAttr target,
  ...
  pipelineAttrs.emplace_back(
      StringAttr::get(linalgOp->getContext(),
                      TTS::GPU::GPUPipelineOptionsAttr::getDictKeyName()),
      pipelineOptions);
  auto pipelineConfig =
