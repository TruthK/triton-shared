...
#define DEBUG_TYPE "tts-codegen-gpu-transforms"
...
getEquivalentMappingConsumerLoopNest(scf::ForallOp producer,
  ...
  if (!llvm::all_of(producerMapping,
      ...
      !llvm::all_of(consumerMapping, llvm::IsaPred<tts::GPU::LaneIdAttr>)) {
    return failure();
  }
...
LogicalResult fuseForallIntoConsumer(RewriterBase &rewriter,
  ...
  // Step 3. Create the `tts_gpu.barrier_region` to wrap the fused producer.
  auto barrierOp = rewriter.create<tts::GPU::BarrierRegionOp>(
  ...
  rewriter.create<tts::GPU::YieldOp>(loc, newProducer.getResults());
...
namespace {
struct LowerMultiMmaPattern : public OpRewritePattern<tts::GPU::MultiMmaOp> {
  using OpRewritePattern<tts::GPU::MultiMmaOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(tts::GPU::MultiMmaOp mmaOp,
...
convertContractionToMultiMma(RewriterBase &rewriter, linalg::LinalgOp linalgOp,
  ...
  auto newMmaOp = rewriter.replaceOpWithNewOp<tts::GPU::MultiMmaOp>(
...
FailureOr<Operation *>
distributeMultiMmaOp(RewriterBase &rewriter, tts::GPU::MultiMmaOp mmaOp,
  ...
  FailureOr<tts::GPU::MMAScope> mmaScope = mmaOp.getKind().getMmaScope();
  if (failed(mmaScope)) {
    return failure();
  }
  switch (mmaScope.value()) {
  case tts::GPU::MMAScope::Workgroup:
    if (!workgroupSize) {
    ...
    break;
  case tts::GPU::MMAScope::Subgroup:
    ub = rewriter.getIndexAttr(mmaOp.getKind().getSubgroupSize());
    mappingType = tts::GPU::LaneIdAttr::get(context, 0);
  }
  ...
  if (failed(mmaOp.getKind().populateOperandOffsetsSizesStrides(
          rewriter, loc, tts::GPU::MMAFragment::Lhs, id, lhsPermutation,
  ...
  if (failed(mmaOp.getKind().populateOperandOffsetsSizesStrides(
          rewriter, loc, tts::GPU::MMAFragment::Rhs, id, rhsPermutation,
  ...
  if (failed(mmaOp.getKind().populateOperandOffsetsSizesStrides(
          rewriter, loc, tts::GPU::MMAFragment::Acc, id, accPermutation,
  ...
  auto newMmaOp = rewriter.create<tts::GPU::MultiMmaOp>(
...
struct DropMultiMmaUnitDimsPattern
    : public OpRewritePattern<tts::GPU::MultiMmaOp> {
  using OpRewritePattern<tts::GPU::MultiMmaOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(tts::GPU::MultiMmaOp mmaOp,
                                PatternRewriter &rewriter) const override {
    if (mmaOp.hasTensorSemantics()) {
...
static bool isReductionIterator(Attribute attr) {
  return cast<tts::GPU::IteratorTypeAttr>(attr).getValue() ==
...
gpuMultiMmaUnrollOrder(Operation *op) {
  tts::GPU::MultiMmaOp mmaOp = dyn_cast<tts::GPU::MultiMmaOp>(op);
  if (!mmaOp) {
    return std::nullopt;
  }
...
static bool isLaneMappableForall(scf::ForallOp forallOp) {
  ...
  if (mapping != tts::GPU::LaneIdAttr::get(forallOp.getContext(), 0)) {
    return false;
  }
  return true;
}
...
struct LowerBarrierRegion
    : public OpRewritePattern<tts::GPU::BarrierRegionOp> {
  using OpRewritePattern<tts::GPU::BarrierRegionOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(tts::GPU::BarrierRegionOp barrierRegionOp,
                                PatternRewriter &rewriter) const final {
    Location loc = barrierRegionOp.getLoc();

    // Step 1. Synchronize the workers on the shared dest.
    auto writeBarrier = rewriter.create<tts::GPU::ValueBarrierOp>(
    ...
    auto barrier = rewriter.create<tts::GPU::ValueBarrierOp>(
...
struct VectorizeStaticMultiMmaOpPattern final
    : OpRewritePattern<tts::GPU::MultiMmaOp> {
  using OpRewritePattern<tts::GPU::MultiMmaOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(tts::GPU::MultiMmaOp mmaOp,
                                PatternRewriter &rewriter) const override {
...
struct LowerValueBarrierPattern
    : public OpRewritePattern<tts::GPU::ValueBarrierOp> {
  using OpRewritePattern<tts::GPU::ValueBarrierOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(tts::GPU::ValueBarrierOp barrier,
                                PatternRewriter &rewriter) const override {
