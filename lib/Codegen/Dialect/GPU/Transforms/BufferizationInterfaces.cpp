...
/// Bufferization of tts_gpu.barrier_region. Always just bufferizes in place
/// and gets inlined with barriers.
struct BarrierRegionOpBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          BarrierRegionOpBufferizationInterface, tts::GPU::BarrierRegionOp> {
  ...
  getAliasingValues(Operation *op, OpOperand &opOperand,
    ...
    auto barrierOp = cast<tts::GPU::BarrierRegionOp>(op);
    alist.push_back(
  ...
  getAliasingOpOperands(Operation *op, Value value,
    ...
    if (auto opResult = dyn_cast<OpResult>(value)) {
      ...
      auto yieldOp =
          cast<tts::GPU::YieldOp>(barrierOp.getBody()->getTerminator());
...
/// Bufferization of tts_gpu.tensor_barrier. Always just bufferizes in place
/// and replaces with a barrier.
struct ValueBarrierOpBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          ValueBarrierOpBufferizationInterface, tts::GPU::ValueBarrierOp> {
  bool bufferizesToMemoryRead(Operation *op, OpOperand &opOperand,
                              const AnalysisState &state) const {
  ...
  getBufferType(Operation *op, Value value, const BufferizationOptions &options,
                SmallVector<Value> &invocationStack) const {
    auto barrierOp = cast<tts::GPU::ValueBarrierOp>(op);
...
/// Bufferization of tts_gpu.yield. Bufferized as part of their enclosing ops,
/// so this is for analysis only.
struct YieldOpBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          YieldOpBufferizationInterface, tts::GPU::YieldOp> {
  bool bufferizesToMemoryRead(Operation *op, OpOperand &opOperand,
                              const AnalysisState &state) const {
    return false;
  }
  ...
  getAliasingValues(Operation *op, OpOperand &opOperand,
                    const AnalysisState &state) const {
    assert(isa<tts::GPU::BarrierRegionOp>(op->getParentOp()));
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    auto yieldOp = cast<tts::GPU::YieldOp>(op);
    ...
    bufferization::replaceOpWithNewBufferizedOp<tts::GPU::YieldOp>(
...
void registerIREEGPUBufferizationInterfaces(DialectRegistry &registry) {
  registry.addExtension(
      +[](MLIRContext *context, tts::GPU::IREEGPUDialect *dialect) {
        tts::GPU::BarrierRegionOp::attachInterface<
            BarrierRegionOpBufferizationInterface>(*context);
        tts::GPU::ValueBarrierOp::attachInterface<
            ValueBarrierOpBufferizationInterface>(*context);
        tts::GPU::YieldOp::attachInterface<YieldOpBufferizationInterface>(
            *context);
      });
}
