...
#define DEBUG_TYPE "tts-gpu-attrs"
#define DBGS() (llvm::dbgs() << "[" DEBUG_TYPE "]: ")
#define LDBG(X) LLVM_DEBUG(DBGS() << X << "\n")

#include "triton-shared/Codegen/Dialect/GPU/IR/TTSGPUEnums.cpp.inc"
#define GET_ATTRDEF_CLASSES
#include "triton-shared/Codegen/Dialect/GPU/IR/TTSGPUAttrs.cpp.inc"
...
static int getBlockSize(MMAIntrinsic /*intrinsic*/) { return 1; }
...
int64_t MMAAttr::getBlockSize() const {
  return tts::GPU::getBlockSize(getIntrinsic().getValue());
}
...
FailureOr<tts::GPU::MMAScope> MMAAttr::getMmaScope() const {
  return tts::GPU::MMAScope::Subgroup;
}
...
LogicalResult MMAAttr::populateOperandOffsetsSizesStrides(
    OpBuilder &builder, Location loc, tts::GPU::MMAFragment fragment,
...
FailureOr<tts::GPU::MMAScope> DataTiledMMAAttr::getMmaScope() const {
  return tts::GPU::MMAScope::Workgroup;
}
...
FailureOr<tts::GPU::MMAScope> VirtualMMAAttr::getMmaScope() const {
  return tts::GPU::MMAScope::Subgroup;
}
...
void TTSGPUDialect::registerAttributes() {
  addAttributes<
#define GET_ATTRDEF_LIST
#include "triton-shared/Codegen/Dialect/GPU/IR/TTSGPUAttrs.cpp.inc" // IWYU pragma: keep
      >();
}
