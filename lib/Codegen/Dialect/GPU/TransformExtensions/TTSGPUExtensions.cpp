#include "triton-shared/Codegen/Dialect/GPU/TransformExtensions/TTSGPUExtensions.h"
...
transform_dialect::TTSGPUExtensions::TTSGPUExtensions() {
  registerTransformOps<
#define GET_OP_LIST
#include "triton-shared/Codegen/Dialect/GPU/TransformExtensions/TTSGPUExtensionsOps.cpp.inc"
      >();
}
...
void transform_dialect::ApplyDropMultiMmaOpUnitDims::populatePatterns(
  ...
  tts::GPU::populateTTSGPUDropUnitDimsPatterns(patterns);
}
...
void transform_dialect::ApplyLowerMultiMmaOp::populatePatterns(
    RewritePatternSet &patterns) {
  tts::GPU::populateTTSGPULowerMultiMmaPatterns(patterns);
}
...
void transform_dialect::ApplyLowerBarrierRegionPatternsOp::populatePatterns(
    RewritePatternSet &patterns) {
  GPU::populateTTSGPULowerBarrierRegionPatterns(patterns);
}
...
void transform_dialect::ApplyLowerValueBarrierOp::populatePatterns(
    RewritePatternSet &patterns) {
  tts::GPU::populateTTSGPULowerValueBarrierPatterns(patterns);
}
...
void transform_dialect::ApplyUnrollMultiMmaOp::populatePatterns(
    RewritePatternSet &patterns) {
  GPU::populateTTSGPUVectorUnrollPatterns(patterns);
}

//===---------------------------------------------------------------------===//
// ApplyVectorizeTTSGPUOp
//===---------------------------------------------------------------------===//

void transform_dialect::ApplyVectorizeTTSGPUOp::populatePatterns(
    RewritePatternSet &patterns) {
  tts::GPU::populateTTSGPUVectorizationPatterns(patterns);
}
...
DiagnosedSilenceableFailure transform_dialect::DistributeMultiMmaOp::applyToOne(
    ...
    transform::ApplyToEachResultList &results,
    transform::TransformState &state) {
  auto mmaOp = dyn_cast<tts::GPU::MultiMmaOp>(target);
  if (!mmaOp) {
    return mlir::emitDefiniteFailure(target, "target is not a multi_mma op");
  }
  rewriter.setInsertionPoint(mmaOp);
  auto maybeForall = tts::GPU::distributeMultiMmaOp(rewriter, mmaOp);
...
DiagnosedSilenceableFailure transform_dialect::ForallToLanesOp::applyToOne(
    ...
    transform::ApplyToEachResultList &results,
    transform::TransformState &state) {
  tts::GPU::mapLaneForalls(rewriter, target, /*insertBarrier=*/true);
  return DiagnosedSilenceableFailure::success();
}
...
void mlir::tts::registerTransformDialectTTSGPUExtension(
    DialectRegistry &registry) {
  registry.addExtensions<
      mlir::tts::transform_dialect::TTSGPUExtensions>();
}
