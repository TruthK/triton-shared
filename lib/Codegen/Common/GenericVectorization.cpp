...
#include "triton-shared/Codegen/Common/TileSizeSelection.h"
...
inferSizesFromIR(linalg::LinalgOp linalgOp, std::optional<OpResult> opResult) {
  ...
  if (!opResult) {
    ...
    auto targetAttr = IREE::GPU::ExecutableTargetAttr::lookup(linalgOp);
...
void GenericVectorizationPass::runOnOperation() {
...

  {
    ...
    auto vscaleRange = mlir::tts::getDefaultVscaleRange(targetAttr);
