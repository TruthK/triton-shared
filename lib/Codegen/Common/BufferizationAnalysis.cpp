...
//
//===----------------------------------------------------------------------===//
#include "triton-shared/Codegen/Common/BufferizationAnalysis.h"
...
static bool canUsersHandleSubviews(Operation *op) {
  ...
  for (Operation *user : op->getUsers()) {
    if (isa<tensor::CollapseShapeOp, tensor::ExpandShapeOp>(user)) {
      return false;
    }
  }
  return true;
    ...
    return TypeSwitch<Operation *, LogicalResult>(op)
        ...
        .Default([&](Operation *op) -> LogicalResult {
          if (llvm::any_of(
             ...
             op->emitWarning("unhandled tensor operation");
             return success();
          }
          return success();
        });
