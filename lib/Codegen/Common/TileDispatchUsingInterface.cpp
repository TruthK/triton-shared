...
// /// Replace the `flow.dispatch.tensor.store` of the `untiledValue` with a
// tiled
...
//     auto storeOp =
//     dyn_cast<IREE::Flow::DispatchTensorStoreOp>(use.getOwner()); if (storeOp
//     && storeOp.getValue() == use.get()) {
...
// /// Replace all `flow.dispatch.tensor.store` operations that use values
// produced
// /// by `untiledOp` as source, with tiled stores, with tiled values produced
// by
// /// `tiledOp`.
...
//     if (failed(untiledOp.getResultTilePosition(rewriter, index, offsets,
//     sizes,
...
// /// Pattern to swap `flow.dispatch.tensor.load` -> `tensor.extract_slice`
// with
...
//     SmallVector<OpFoldResult> combinedOffsets, combinedSizes,
//     combinedStrides; if (failed(affine::mergeOffsetsSizesAndStrides(
//             rewriter, loadOp.getLoc(), loadOp, sliceOp,
//             loadOp.getDroppedDims(), combinedOffsets, combinedSizes,
//             combinedStrides))) {
//       return rewriter.notifyMatchFailure(
//           sliceOp, "failed to fold offsets, sizes and strides with load op");
//     }
//     rewriter.replaceOpWithNewOp<IREE::Flow::DispatchTensorLoadOp>(
//         sliceOp, sliceOp.getType(), loadOp.getSource(),
...
void populateTileAndDistributeToWorkgroupsCleanupPatterns(
  ...
  patterns.insert<SwapExtractSliceWithTensorEmpty>(context);
}
