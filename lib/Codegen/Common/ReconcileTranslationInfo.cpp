...
getProcIdsAndNprocs(
  ...
  for (int index = workgroupMappings.size() - 1; index >= 0; --index) {
    ...
    case IREE::Codegen::WorkgroupId::IdX:
      procId[index] =
          builder.create<gpu::BlockIdOp>(loc, gpu::Dimension::x).getResult();
      nprocs[index] =
          builder.create<gpu::GridDimOp>(loc, gpu::Dimension::x).getResult();
      break;
    case IREE::Codegen::WorkgroupId::IdY:
      procId[index] =
          builder.create<gpu::BlockIdOp>(loc, gpu::Dimension::y).getResult();
      nprocs[index] = builder.create<gpu::GridDimOp>(loc, gpu::Dimension::y).getResult();
      break;
  ...
  if (!loopExtents.empty()) {
    Value zDimId = builder.create<gpu::BlockIdOp>(loc, gpu::Dimension::z).getResult();
    OpFoldResult zNprocs = builder.create<gpu::GridDimOp>(loc, gpu::Dimension::z).getResult();
...
void ReconcileTranslationInfoPass::runOnOperation() {
  auto innerModuleOp = getOperation();
  ...
  if (failed(reconciledWorkgroupSize)) {
    innerModuleOp.emitOpError("failed to reconcile workgroup sizes");
    return signalPassFailure();
  }
  ...
  innerModuleOp->setAttr("hal.workgroup_size", workgroupSizeArrayAttr);
  ...
  if (failed(reconciledSubgroupSize)) {
    innerModuleOp.emitOpError("failed to reconcile subgroup size");
    return signalPassFailure();
  }
  if (reconciledSubgroupSize.value() != int64_t()) {
    innerModuleOp->setAttr("hal.subgroup_size",
