...
#define DEBUG_TYPE "iree-codegen-gpu-distribute-vector-ext"

namespace mlir::tts {

#define GEN_PASS_DEF_GPUDISTRIBUTEVECTOREXTPASS
...
static int getBaseVectorSize(IREE::VectorExt::TransferWriteOp transferWriteOp) {
  unsigned resultBW =
      llvm::cast<MemRefType>(transferWriteOp.getValue().getType())
  ...
  for (OpOperand &operand : transferWriteOp->getOpOperands()) {
...
getTileToDistributableSize(IREE::VectorExt::TransferWriteOp transferWriteOp,
                           int64_t flatWorkgroupSize) {
  auto shape = transferWriteOp.getValue().getType().getShape();
  int targetVectorSize = getBaseVectorSize(transferWriteOp);
...
static LogicalResult tileToUnroll(mlir::FunctionOpInterface funcOp,
  ...
  funcOp.walk([&](IREE::VectorExt::TransferWriteOp transferWriteOp) {

    builder.setInsertionPointAfter(transferWriteOp);
    SmallVector<Value> tileSizesVal;
    std::optional<SmallVector<int64_t>> staticSize =
        getTileToDistributableSize(transferWriteOp, flatWorkgroupSize);
    for (int64_t dim : *staticSize) {
      tileSizesVal.push_back(
          builder.create<arith::ConstantIndexOp>(transferWriteOp->getLoc(), dim));
    }

    TilingInterface op = dyn_cast<TilingInterface>(transferWriteOp);
    FailureOr<TilingResult> tiledImplementation =
        op.getTiledImplementation(builder, offsets, tileSizesVal);
  });
}
...
LogicalResult gpuDistributeVectorExt(mlir::FunctionOpInterface funcOp) {
  ...
  MLIRContext *context = funcOp.getContext();
  SmallVector<IREE::VectorExt::TransferWriteOp> copiesToWorkgroupMem;
  funcOp.walk([&](IREE::VectorExt::TransferWriteOp copyOp) {
  ...
  bool isAligned = llvm::all_of(
      copiesToWorkgroupMem,
      [flatWorkgroupSize](IREE::VectorExt::TransferWriteOp copyOp) {
        auto vecType = cast<VectorType>(copyOp.getValue().getType());
        SmallVector<int64_t> shape(vecType.getShape().begin(),
                                   vecType.getShape().end());
        int elementBitWidth = getTypeBitWidth(vecType.getElementType());
        int targetVectorSize = copyVectorNumBits / elementBitWidth;
...
namespace {
struct GPUDistributeVectorExtPass final
    : impl::GPUDistributeVectorExtPassBase<GPUDistributeVectorExtPass> {
  void runOnOperation() override {
    auto funcOp = getOperation();
    if (failed(gpuDistributeVectorExt(funcOp))) {
