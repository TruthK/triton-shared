...
getWorkgroupCountsXY(OpBuilder &builder, FunctionOpInterface funcOp) {
  ...
  Value dynamicCountX = builder.create<gpu::GridDimOp>(loc, gpu::Dimension::x);
  Value dynamicCountY = builder.create<gpu::GridDimOp>(loc, gpu::Dimension::y);
...
reorderWorkgroupsInFunc(FunctionOpInterface funcOp,
  ...
  gpu::BlockIdOp oldXId;
  gpu::BlockIdOp oldYId;
  unsigned numXIdOps = 0;
  unsigned numYIdOps = 0;
  funcOp.walk([&](gpu::BlockIdOp idOp) {
    auto index = idOp.getDimension();
    if (index == gpu::Dimension::x) {
      oldXId = idOp;
      ++numXIdOps;
    } else if (index == gpu::Dimension::y) {
      oldYId = idOp;
      ++numYIdOps;
    } else {
      assert(false);
  ...
  Value workgroupIdX = builder.create<gpu::BlockIdOp>(funcOp.getLoc(), gpu::Dimension::x);
  Value workgroupIdY = builder.create<gpu::BlockIdOp>(funcOp.getLoc(), gpu::Dimension::y);
