...
inline llvm::cl::opt<bool>
    clSetTunerAttr("tts-config-add-tuner-attributes",
                   llvm::cl::desc("Adds attribute for tuner."),
                   llvm::cl::init(false));
} // namespace mlir::tts

// clang-format off
#include "triton-shared/Codegen/Dialect/Codegen/IR/LoweringConfigEnums.h.inc"
#define GET_ATTRDEF_CLASSES
#include "triton-shared/Codegen/Dialect/Codegen/IR/TTSCodegenAttrs.h.inc"
// clang-format on
...
constexpr StringLiteral kTuningSpecDefaultEntrypointAttrName =
    "tts_codegen.tuning_spec_with_default_entrypoint";
constexpr StringLiteral kTuningSpecEntrypointAttrName =
    "tts_codegen.tuning_spec_entrypoint";
constexpr StringLiteral kSerializedTuningSpecAttrName =
    "tts_codegen.tuning_spec_mlirbc";
...
setTranslationInfo(mlir::FunctionOpInterface entryPoint,
                   tts::GPU::TranslationInfoAttr translationInfo);
...
template <typename ConfigTy = tts::GPU::LoweringConfigAttrInterface>
...
/// `tts_codegen.lowering_config` attribute is set on it.
...
inline LogicalResult setOpConfigAndEntryPointFnTranslation(
    ...
    tts::GPU::LoweringConfigAttrInterface config,
    tts::GPU::TranslationInfoAttr translationInfo) {
  if (clSetTunerAttr) {
    setRootOpInfo(op);
  }
...
inline LogicalResult setOpConfigAndEntryPointFnTranslation(
    mlir::FunctionOpInterface entryPointFn, Operation *op,
    tts::GPU::LoweringConfigAttrInterface config,
    tts::GPU::DispatchLoweringPassPipeline passPipeline,
    ArrayRef<int64_t> workgroupSize = {},
    std::optional<int64_t> subgroupSize = {},
    DictionaryAttr pipelineConfig = DictionaryAttr()) {
  MLIRContext *context = entryPointFn.getContext();
  auto translationInfo = tts::GPU::TranslationInfoAttr::get(
...
inline LogicalResult setOpConfigAndEntryPointFnTranslation(
    ...
    DictionaryAttr pipelineConfig = DictionaryAttr()) {
  MLIRContext *context = entryPointFn.getContext();
  auto config = tts::GPU::LoweringConfigAttr::get(context, tileSizes,
                                                       scalableTileFlags);
  return setOpConfigAndEntryPointFnTranslation(entryPointFn, op, config,
...
/// Returns the `#tts_codegen.compilation_info` set on the operation. Assumes
/// that the identifier used is `compilation_info`.
tts::GPU::CompilationInfoAttr getCompilationInfo(Operation *op);
...
void setCompilationInfo(Operation *op,
                        tts::GPU::CompilationInfoAttr config);

/// Removes the `#tts_codegen.compilation_info` attribute that is set on the
/// operation.
